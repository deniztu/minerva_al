...
We created cue, outcome, and context vectors and defined the number of trials and replications according to the specifications in the paper.
...
The results correspond nicely to those reported in Figure 1.
...
For comparison, the following plot shows the results from the reference Fortran implementation we received from Randall Jamieson (thanks!).
...
For this simulation, we used the same post-extinction memory for the reacquisition and control condition to reduce differences due to random variation during the initial acquisition and extinction trials.
The following figure visualizes the changes to the retrieval of X given the cue during the third (re-)acquisition phase.
The dashed lines represent the control condition, the solid lines the reacquisition condition.
...
knitr::kable(
  cbind(Condition = c("Reacquisition", "Control"), reacquisition_results)
  , col.names = c("Condtion", round(p_encode, 2))
  , caption = "Reproduction of Table 1"
)
```

Our results diverge quite a bit from those originally reported whenever the learning rate is not perfect.

```{r}
original <- matrix(
  c("36.2 (5.9)", "62.2 (7.4)", "19.7 (5.1)", "32.1 (6.4)", "5.1 (0.2)", "3.8 (0.1)")
  , ncol = 3
)
knitr::kable(
  cbind(Condition = c("Reacquisition", "Control"), original)
  , col.names = c("Condtion", round(p_encode, 2))
  , caption = "Table 1 as reported by Jamieson et al. (2012)"
)
```

For comparison, we also implemented the reacquisition protocol in Fortran, based on the reference implementation for the simulation of acqusition and extinction.
In this simulation we used different post-extinction memories.
The results correspond closely to the results from our previous reproduction efforts.
...
knitr::kable(
  ...
  , caption = "Reproduction of Table 1 based on Fortran implementation"
)
```
